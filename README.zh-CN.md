...
DataVision 是一个面向企业团队的开源 BI 看板托管平台，核心场景是：你在本地用 coding agent、Python、SQL 或任意前端工具生成一个 HTML 看板，然后把它上传到 DataVision，团队成员就可以通过权限受控的链接直接查看。
...
![DataVision 概览](public/demo/overview.png)
...
- 内置 `datavision` CLI、同步 Worker 和 API smoke test。
...
第一次请求会自动初始化演示组织、用户、文件夹、数据集、精美示例看板和 `.datavision` 本地运行数据。
...
零仓库用户接入流程见 [docs/ONBOARDING.zh-CN.md](docs/ONBOARDING.zh-CN.md)。可复制的 Claude Code skill 模板见 [templates/claude-code-datavision-publisher/SKILL.md](templates/claude-code-datavision-publisher/SKILL.md)。维护者发布 CLI 的步骤见 [docs/CLI-RELEASE.md](docs/CLI-RELEASE.md)。
...
| `DATA_DIR` | `.datavision` | 本地元数据目录，也是默认 SQLite 父目录。 |
| `SQLITE_PATH` | `.datavision/datavision.sqlite` | `DATA_DRIVER=sqlite` 时的数据库路径。 |
| `UPLOAD_DIR` | `.datavision/uploads` | 上传的看板和数据集文件目录。 |

## 代码结构

- `app/`：Next.js App Router 页面、预览页和 REST API。
- `lib/server/`：认证、权限、数据库 adapter、文件存储、序列化、数据集解析和同步执行器。
- `packages/cli/`：对外发布的独立 CLI 包；`bin/datavision.mjs` 保留为仓库内包装入口。
- `scripts/sync-worker.mjs`：基于 API Key 的同步 Worker。
- `.datavision/`：本地运行数据，不提交到 Git。
