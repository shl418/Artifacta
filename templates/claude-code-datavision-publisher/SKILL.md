---
name: datavision-publisher
description: Use when the user wants Claude Code to publish a locally generated HTML dashboard, ZIP bundle, dataset file, or sync configuration JSON to a running DataVision site. Triggers include "publish to datavision", "push this dashboard", "上传到 datavision", "把这个看板发到网页上", "upload this html", "推送 zip 看板", and "用 skill 上传数据和配置".
---

# DataVision Publisher

Use this skill when Claude Code should turn local output files into a remote DataVision project.

This skill is specifically for the zero-repo case:

- the user does not need the DataVision app repository locally
- the user only needs a DataVision URL, an API key, and local output files
...
1. Confirm the target DataVision site and API key are available as environment variables.
2. Write generated artifacts to a temp directory.
3. If no `project_id` is provided, create a remote project with `datavision projects upload`.
4. If `project_id` exists, replace the dashboard HTML with `datavision projects update-html`.
5. Upload or replace datasets as needed.
6. If a sync config JSON is provided, call `datavision datasets sync set`.
...
1. `datavision ...` when the standalone CLI is already installed
2. `pnpm cli -- ...` when the current working directory is the DataVision repository
3. `npx @datavision/cli@latest ...` after the maintainer has published the npm package
...
- Treat `project_id` and `dataset_id` as remote IDs returned by DataVision.
...
- If `DATAVISION_API_KEY` is missing, stop and ask for it.
