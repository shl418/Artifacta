...
Artifacta 相信另一件事：**让 AI 直接写 HTML 看板**——图表、叙事、交互一次成型，比拖控件快一个数量级；发布后还可以直接在浏览器里原位修改文字，不必为了一个标题重新打包上传。
...
AI 生成的看板不再是上传后就难以修改的静态成品。拥有编辑权限的成员可以在预览页点击“编辑文字”，直接选中看板里的标题、指标、注释等可见文字，在原位置完成修改和预览。

![Artifacta 在线 HTML 看板文字编辑器](public/demo/online-html-editor.png)

- **所见即所得：** 点击页面文字即可编辑，同时保留原有排版、图表和交互。
- **修改可控：** 集中查看修改前后内容，支持逐项撤销、整体撤销 / 重做和本地草稿恢复。
- **版本化发布：** 点击“保存并发布”会生成新的不可变版本，历史版本仍可查看和回滚。
- **多人协作：** 保存时如果服务器已有新版本，系统会自动合并无冲突内容，并提供逐项冲突处理界面。
- **保持隔离：** 编辑和预览继续运行在沙箱 iframe 中，不会把上传的 HTML 提升到主应用权限。

## 当前已经可用

- 上传单文件 `.html` 看板，或上传包含 CSS、JS、图片等资源的 `.zip` 看板包。
- 通过沙箱 iframe 预览看板，支持私有、团队、公开三种可见性。
- 在浏览器中原位修改 HTML 看板的可见文字，支持草稿、撤销 / 重做、版本发布和并发合并。
